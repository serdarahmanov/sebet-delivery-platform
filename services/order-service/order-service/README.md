...
The current codebase contains the API/DTO/cache skeleton, durable order persistence, Flyway schema migration, repository tests, an internal order creation service, checkout-event-to-command mapping, Kafka retry/DLT handling, a Kafka checkout-event consumer, Redis locking around checkout order creation, Redis hot-view writes during order creation, order event outbox writes for Debezium publishing, idempotent driver assignment and store cancellation writes, deliberate Redis C2 eviction fallback events, customer read/write services, store read/write services, driver detail and lifecycle endpoints (detail, pickup, arrive, complete, decline), internal driver assignment endpoints (assign, replace, unassign), internal lifecycle endpoints, ShedLock-guarded scheduled-order activation, and a global exception handler with consistent error responses. WebSocket broker, timeout background jobs, delivery tracking integration, and deployment/runtime wiring are still pending.
