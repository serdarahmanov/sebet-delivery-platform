...
The current codebase contains the API/DTO/cache skeleton, durable order persistence, Flyway schema migration, repository tests, an internal order creation service, checkout-event-to-command mapping, Kafka retry/DLT handling, a Kafka checkout-event consumer, Redis locking around checkout order creation, Redis hot-view writes during order creation, order event outbox writes for Debezium publishing, idempotent driver assignment and store cancel writes, deliberate Redis C2 eviction fallback events, customer read services, store read services, store lifecycle write endpoints (accept, reject, ready, cancel), driver detail and lifecycle endpoints (detail, pickup, arrive, complete, decline), internal driver assignment endpoints (assign, replace, unassign), and a global exception handler with consistent error responses. WebSocket broker, remaining REST write service implementations (customer writes, store propose-changes, remaining internal endpoints), and Debezium connector deployment/runtime wiring are still pending.
...
- [Debezium Outbox](docs/14-debezium-outbox.md)
