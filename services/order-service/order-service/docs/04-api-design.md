...
- `POST /api/v1/internal/orders/{orderId}/update-after-proposal` - promo-service callback applies recalculated totals/items and returns the order to `CONFIRMED`
...
| `POST /api/v1/store/orders/{orderId}/cancel-active-proposal` | cancel active proposal only, allowing a corrected proposal later | implemented |
...
| `POST /api/v1/internal/orders/{orderId}/update-after-proposal` | promo-service callback applies recalculated totals/items and moves accepted proposal flow back to `CONFIRMED` | implemented |

Assignment writes are idempotent by `Idempotency-Key`. The key is first reserved as `IN_PROGRESS`; the owning request then writes the order update, assignment outbox event, and stored idempotent response. After commit, order-service tries to evict C2 (`order:{orderId}`) so the next Redis-first read rebuilds from PostgreSQL. If Redis is unavailable or the Redis command result is unknown, order-service writes a deliberate `OrderCacheEvictionRequested` outbox event and still returns `200`. Non-Redis runtime failures propagate normally. If that fallback event cannot be recorded, the endpoint returns `503 CACHE_INVALIDATION_FAILED`; callers should retry the same request with the same idempotency key. Reusing the same key while the original request is still running returns `409 IDEMPOTENCY_REQUEST_IN_PROGRESS`; reusing it for a different request body returns `409 IDEMPOTENCY_KEY_CONFLICT`.

Scheduled activation is also exposed as an admin/support manual activation endpoint. It is idempotent by `Idempotency-Key`. The key is first reserved as `IN_PROGRESS`; the owning request then writes the status transition, `OrderActivated` outbox event, and stored idempotent response. After commit and on idempotent replay, it atomically removes the order from Cache 1c, adds it to Cache 1 and Cache 1b, and writes Cache 4 through a Lua script. Recoverable Redis failures record an `OrderCacheEvictionRequested` event for `SCHEDULED_ACTIVATION_HOT_VIEWS`; non-Redis runtime failures still propagate. The automatic scheduled-order activation job uses an env-configured lead time, interval, initial delay, and batch size.
...
System cancel is idempotent by `Idempotency-Key`. The endpoint is for automated internal flows and accepts only system-owned reasons: `PAYMENT_FAILED`, `NO_RIDERS_AVAILABLE`, `STORE_RESPONSE_TIMEOUT`, `AWAITING_CUSTOMER_RESPONSE_TIMEOUT`, and `SYSTEM_ERROR`. It can cancel `PENDING`, `CONFIRMED`, `AWAITING_CUSTOMER_RESPONSE`, `SCHEDULED`, or `READY_FOR_PICKUP` orders, and rejects `OUT_FOR_DELIVERY`, `ARRIVED`, `DELIVERED`, and already `CANCELLED` orders. Both cancellation paths reserve the idempotency key first, then the owning request writes the cancellation status, status history, lifecycle outbox event, and stored idempotent response. After commit and on idempotent replay, Redis cleanup uses `CANCELLED_ORDER_HOT_VIEWS`, which removes C1/C1b membership and deletes C2/C3/C4/C6 through the recoverable eviction fallback pattern.

Store and internal `cancel-active-proposal` are idempotent by `Idempotency-Key`. The store endpoint also verifies `X-Store-Id` ownership. The action requires the order to be `AWAITING_CUSTOMER_RESPONSE` and requires a durable active proposal row. The database transaction marks the proposal row as `CANCELLED` (retained as an audit record), moves the order back to `CONFIRMED`, writes status history, emits `OrderActiveProposalCancelled`, and stores the idempotent command response. After commit and on idempotent replay, a Redis Lua script deletes C8, writes C4 as `CONFIRMED`, and removes `AWAITING_CUSTOMER_RESPONSE` entries from C6 without removing the rest of the timeline. Recoverable Redis failures write an `OrderCacheEvictionRequested` event for `CANCEL_ACTIVE_PROPOSAL_HOT_VIEWS`.

Internal `cancel-proposal-and-order` is idempotent by `Idempotency-Key`. It requires the order to be `AWAITING_CUSTOMER_RESPONSE`. The database transaction marks the proposal row as `TIMED_OUT` (retained as an audit record), transitions the order to `CANCELLED` with reason `AWAITING_CUSTOMER_RESPONSE_TIMEOUT`, writes status history, emits `OrderCancelled`, and stores the idempotent command response. After commit and on idempotent replay, Redis cleanup uses `CANCELLED_ORDER_HOT_VIEWS`, which removes C1/C1b membership and deletes C2/C3/C4/C6/C8 through the recoverable eviction fallback pattern. The automatic `ProposalTimeoutScheduler` job calls the same underlying transition (bypassing the `Idempotency-Key` REST layer, relying instead on the `AWAITING_CUSTOMER_RESPONSE` status guard for at-most-once DB effect) for orders whose proposal has been outstanding longer than the env-configured response window, in env-configured batches on an interval, guarded by PostgreSQL-backed ShedLock.

Internal `update-after-proposal` is idempotent by `Idempotency-Key`. It is called by the promo service after the customer accepts a store proposal and discounts/totals have been recalculated. The request must reference the accepted `proposalId`, include the order-level `currency`, final totals, selected promo codes, and the final repriced order-item snapshot. The database transaction validates that the order is still `AWAITING_CUSTOMER_RESPONSE`, the proposal is `ACCEPTED`, and the currency matches the order; then it replaces durable order items, updates totals, marks the proposal `APPLIED`, transitions the order to `CONFIRMED`, writes status history, emits `OrderProposalApplied`, and stores the idempotent response. After commit and on idempotent replay, a Redis Lua script deletes stale C2, deletes C8, writes C4 as `CONFIRMED`, and removes `AWAITING_CUSTOMER_RESPONSE` entries from C6. Recoverable Redis failures write an `OrderCacheEvictionRequested` event for `APPLY_PROPOSAL_HOT_VIEWS`.
...
- `409 Conflict`: invalid lifecycle transition, modification outside allowed window, reused `Idempotency-Key` with a different request, or retry of an idempotent request that is still `IN_PROGRESS`.
